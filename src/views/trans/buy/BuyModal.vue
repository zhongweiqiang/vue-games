<template>
  <div style="display: inline;">
    <!-- <a-icon type="money" @click="showModal" />供货 -->
    <a-button type="primary" size="small" @click="showModal">供货</a-button>
    <a-modal
      title="供货"
      :visible="visible"
    ...
    >
      <a-form :form="form" @submit="handleSubmit">
        ...
        <a-form-item label="供货数量" :label-col="{ span: 5 }" :wrapper-col="{ span: 18 }">
          <a-input-number
            :min="1"
            :max="info.unit"
            @change="onChange"
            v-decorator="['unit', { rules: [{ required: true, message: '数量必须填写且为数字', type: 'number', transform(value) {
                      if(value){
                        return Number(value);
                      }
                    }, }] }]"
